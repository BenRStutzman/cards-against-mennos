...
def exchange(message):
    ...
    f = open('Lib\\site-packages\\py_to_js.txt', 'w')
    f.write(message)
    f.close()
    response = read_and_reset('Lib\\site-packages\\js_to_py.txt')
    return response
...
server = threading.Thread(target = os.system, args = {'"Lib\\site-packages\\flask" run'})
server.start()
